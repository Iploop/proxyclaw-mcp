...
function curlThroughProxy(url, opts = {}) {
    return new Promise((resolve) => {
        let auth = `:${API_KEY}`;
        if (opts.country)
            auth += `-country-${opts.country.toUpperCase()}`;
        if (opts.city)
            auth += `-city-${opts.city.toLowerCase()}`;
        if (opts.session)
            auth += `-session-${opts.session}`;
        if (opts.asn)
            auth += `-asn-${opts.asn}`;
        const args = [
...
const TOOLS = [
    {
        name: "proxy_fetch",
        description: "Fetch a URL through the ProxyClaw residential proxy network. " +
    ...
    {
        name: "proxy_check_ip",
        description: "Check the current exit IP address and geo-location when routing through the proxy. " +
    ...
    },
    {
        ...
        description: "List all 195+ countries available for routing through the ProxyClaw residential network.",
        inputSchema: {
            type: "object",
            properties: {},
            required: [],
    ...
    {
        name: "proxy_rotate",
        description: "Force IP rotation by fetching with a new random session. " +
